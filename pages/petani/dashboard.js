...
const PetaniDashboard = () => {
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        try {
          ...
          if (accounts.length > 0) {
            ...
            // Redirect if user is not a Petani (role 1)
            if (Number(role) !== 1) {
              if (Number(role) === 0) {
  ...
  useEffect(() => {
    const fetchDistributions = async () => {
      ...
      try {
        ...
        // Fetch all distributions and filter those created by this farmer
        for (let i = 1; i <= distributionCount; i++) {
          const distributionDetails = await contract.methods.getDistribusi(i).call();
          
          if (distributionDetails.petani.toLowerCase() === walletAddress.toLowerCase()) {
        ...
        distributionsData.sort((a, b) => b.id - a.id);
  ...
  return (
    <Layout>
      <Head>
        <title>Dashboard Petani | Distribusi Kopi</title>
      </Head>
      ...
      <div className="dashboard-container">
        <div className="dashboard-header">
          ...
          <div className="user-info">
            <span className="role-badge">Petani</span>
        ...
        <div className="dashboard-actions">
          <Link href="/distribusi/new">
            <button className="btn-primary">Buat Distribusi Baru</button>
          </Link>
        </div>

        <div className="dashboard-content">
          <h2>Distribusi Kopi Saya</h2>
          
          {distributions.length === 0 ? (
            <div className="no-data">
              <p>Anda belum memiliki distribusi kopi.</p>
              <Link href="/distribusi/new">
                <button className="btn-secondary">Buat Distribusi Baru</button>
              </Link>
            </div>
          ) : (
            <div className="distributions-list">
              {distributions.map((distribution) => (
                <div className="distribution-card" key={distribution.id}>
                  ...
                  <div className="distribution-footer">
                    <Link href={`/distribusi/${distribution.id}`}>
      ...
      <style jsx>{`
        ...
        .btn-secondary:hover {
          transform: translateY(-2px);
          box-shadow: 0 10px 20px rgba(60, 42, 33, 0.3);
        }
